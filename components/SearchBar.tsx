...
export default function SearchBar({ onSearch, onClear, placeholder = 'Ask anything... e.g., "baroque musicians from France"' }: SearchBarProps) {
  ...
  return (
    <div className="relative">
      <div className={`flex items-center gap-2 p-4 rounded-xl border-2 transition-all ${
        isActive
      ...
      {isActive && (
        <div className="mt-2 p-3 bg-purple-100 border-2 border-purple-500 rounded-lg">
          <p className="text-sm font-bold" style={{ color: '#7c3aed', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
            {semanticMode ? '🧠 AI Semantic Search' : '📝 Keyword Search'}: "{query}"
          </p>
          <p className="text-xs mt-1" style={{ color: '#6b7280', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
            {semanticMode
